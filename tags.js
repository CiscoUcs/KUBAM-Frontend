...
riot.tag2('hosts', '<div class="svrGrpServers"> <div class="top-actions"> <fancy-dropdown inputid="actions" class="table-input"> <option value="none">Actions</option> <option value="buildimage">Build server image</option> <option value="deploy">Deploy ALL</option> <option value="deploy">Deploy UCS</option> <option value="deploy">Deploy vmedia policy</option> <option value="deploy">Reset drives</option> </fancy-dropdown> </div> <table-search></table-search> <div class="table"> <div class="tr"> <div class="th checkbox_width"> <input type="checkbox" id="select_all" onclick="{changeSelection}"> </div> <div class="th hostname_width">Hostname</div> <div class="th ip_width">Server IP</div> <div class="th dropdown_width">Operating System</div> <div class="th dropdown_width">Role</div> <div class="th dropdown_width">Network</div> <div class="th">Server</div> <div class="th actionwidth">Actions</div> </div> <div class="tr" each="{host, iindex in this.opts.store.getState().hosts}"> <div class="td-host checkbox_width"> <input type="checkbox" class="hostcheckboxes" checked="{host.name.startsWith(⁗undefined⁗)}"> </div> <div class="td-host hostname_width"> <input type="text" riot-value="{host.name}" data-op="host" data-old="{host.name}" data-index="{iindex}" onblur="{changeHost}"> </div> <div class="td-host ip_width"> <input type="text" riot-value="{host.ip}" data-op="ip" data-old="{host.ip}" data-index="{iindex}" onblur="{changeHost}"> </div> <div id="os_drop" class="td-host "> <table-dropdown default="{host.os}" top="{host.os}" add=""> <li each="{key, value in passStore.getState().catalog}"> <a data-os="{value}" onclick="{switch_os}">{translateOS(value)}</a> </li> </table-dropdown> </div> <div id="role_drop" class="td-host"> <table-dropdown default="{host.role}" top="{translateRole(host.role)}" os="{host.os}"> <li each="{value in passStore.getState().catalog != null ? passStore.getState().catalog[this.opts.os] : []}"> <a data-role="{value}" onclick="{switch_role}">{translateRole(value)}</a> </li> </table-dropdown> </div> <div id="nw_drop" class="td-host"> <table-dropdown default="{host.network_group}" top="{host.network_group}" add=""> <li each="{nw in passStore.getState().networks}"> <a data-nw="{nw.name}" riot-value="{nw.name}" onclick="{switch_network}">{nw.name}</a> </li> </table-dropdown> </div> <div class="td-host server"> </div> <div class="td actionwidth"> <img src="./icons/delete.svg" data-hostname="{host.name}" onclick="{deleteHost}" class="table-icon"> </div> </div> </div> </div> <add-button onclick="{addHost}">Add Host</add-button>', 'hosts .td-host,[data-is="hosts"] .td-host{ padding: 0; font-size: 0.8em; display: table-cell; border: 1px solid #ecedf1; background-color: #FFF; } hosts .checkbox_width,[data-is="hosts"] .checkbox_width{ width: 20px; } hosts .td-host input[type=checkbox],[data-is="hosts"] .td-host input[type=checkbox]{ position: relative; left: 12px; } hosts .server,[data-is="hosts"] .server{ padding: 0; padding-left: 8px; vertical-align: middle; } hosts .hostname_width,[data-is="hosts"] .hostname_width{ width: 140px; } hosts .ip_width,[data-is="hosts"] .ip_width{ width: 120px; } hosts .dropdown_width,[data-is="hosts"] .dropdown_width{ width: 136px; line-height: 1px; } hosts .hostname_width input:hover,[data-is="hosts"] .hostname_width input:hover,hosts .ip_width input:hover,[data-is="hosts"] .ip_width input:hover{ background-image: url(\'icons/edit.svg\'); background-repeat: no-repeat; background-position: right; } hosts .hostname_width input:focus,[data-is="hosts"] .hostname_width input:focus,hosts ip_width input:focus,[data-is="hosts"] ip_width input:focus{ background-image: none; } hosts .top-actions fancy-dropdown,[data-is="hosts"] .top-actions fancy-dropdown{ cursor: pointer; } hosts .svrGrpSettings,[data-is="hosts"] .svrGrpSettings{ background-color: white; padding: 25px; margin-bottom: 20px; clear: both; overflow:auto; } hosts .svrGrpServers,[data-is="hosts"] .svrGrpServers{ background-color: white; padding: 20px; overflow: hidden; min-height: 800px; } hosts .table input[type=text],[data-is="hosts"] .table input[type=text]{ border: none; outline:0; width: 90%; padding: 0; padding-left: 8px; margin: 0; font-size: 1em; cursor: pointer; }', '', function(opts) {
        let currentValue
