...
riot.tag2('infra-overview', '<div class="infra-group"> <div class="infra-container-big"> <div class="containing-actions"> <div class="top-actions"> <fancy-dropdown inputid="actions" class="table-input"> <option value="none">Actions</option> <option value="Edit selected hosts">Edit selected hosts</option> <option value="Delete select hosts">Delete select hosts</option> </fancy-dropdown> <div class="table"> <div class="tr"> <div class="th"><input type="checkbox"></div> <div class="th">Name</div> <div class="th">Description</div> <div class="th">Type</div> <div class="th">Mgmt IP</div> <div class="th">Tenant</div> </div> <div class="tr" each="{comp in this.opts.store.getState().servers}"> <div class="td" style="background-color: white"> <input type="checkbox"> </div> <div class="td">{comp.name}</div> <div class="td">{comp.description}</div> <div class="td">{comp.type}</div> <div class="td">{comp.credentials.ip}</div> <div class="td">{comp.credentials.user}</div> </div> <div class="tr" each="{comp in this.opts.store.getState().aci}"> <div class="td" style="background-color: white"> <input type="checkbox"> </div> <div class="td">{comp.name}</div> <div class="td">{comp.description}</div> <div class="td">{comp.type}</div> <div class="td">{comp.credentials.ip}</div> <div class="td">{comp.credentials.user}</div> </div> </div> </div> </div> </div> <add-button onclick="{addController}">Add Controller</add-button>', 'infra-overview .tablewidth,[data-is="infra-overview"] .tablewidth{ width: 720px; } infra-overview .infra-group,[data-is="infra-overview"] .infra-group{ padding-bottom: 15px; } infra-overview .infra-container,[data-is="infra-overview"] .infra-container{ background-color: white; padding: 20px; } infra-overview .infra-container-big,[data-is="infra-overview"] .infra-container-big{ background-color: white; padding: 34px 20px; }', '', function(opts) {
...
riot.tag2('network', '<add-button onclick="{addNetworkGroup}"> Add Network Group </add-button> <div class="network-group"> <div class="top-actions"> <fancy-dropdown inputid="actions" class="table-input"> <option value="none">Actions</option> <option value="Edit selected hosts">Edit selected hosts</option> <option value="Delete select hosts">Delete select hosts</option> </fancy-dropdown> </div> <div class="network-container-big"> <div class="table"> <div class="tr"> <div class="th"><input type="checkbox"></div> <div class="th">Network Name</div> <div class="th">Netmask</div> <div class="th">Router</div> <div class="th">Name Server</div> <div class="th">NTP Server</div> <div class="th">Proxy Server</div> <div class="th">VLAN</div> </div> <div class="tr" each="{nw in this.opts.store.getState().networks}"> <div class="th" style="background-color: white"><input type="checkbox"></div> <div class="td">{nw.name}</div> <div class="td">{nw.netmask}</div> <div class="td">{nw.gateway}</div> <div class="td">{nw.nameserver}</div> <div class="td">{nw.ntpserver}</div> <div class="td">{nw.proxy}</div> <div class="td">{nw.vlan}</div> </div> </div> </div> </div>', 'network .network-group,[data-is="network"] .network-group{ padding-bottom: 15px; background-color: white; padding: 20px; } network .network-container,[data-is="network"] .network-container{ background-color: white; padding: 20px; }', '', function(opts) {
        let currentValue
        let store = this.opts.store
...
riot.tag2('new-serverimage', '<form> <fancy-dropdown tag="Operating System" inputid="mapping-name"> <option value="centos7.3">CentOS 7.3</option> <option value="centos7.4">CentOS 7.4</option> <option value="esxi6.0">ESXi 6.0</option> <option value="esxi6.5">ESXi 6.5</option> <option value="redhat7.2">RedHat 7.2</option> <option value="redhat7.3">redhat7.3</option> <option value="redhat7.4">redhat7.4</option> </fancy-dropdown> <fancy-dropdown name="ISO" tag="ISO" inputid="mapping-iso"> <option each="{img in passStore.getState().isos}" riot-value="{img}">{img.substring(0,21)}...</option> </fancy-dropdown><br> </form> <fancy-button onclick="{createImageMapping}">Create</fancy-button> <fancy-button color="gray" onclick="{closeModal}">Cancel</fancy-button>', 'new-serverimage form,[data-is="new-serverimage"] form{ text-align: left; }', '', function(opts) {
        passStore.dispatch({
            type: 'FETCH_IMAGES'
        })
...
riot.tag2('serverimages-overview', '<div class="osgroup" hide="{this.opts.store.getState().isLoading}"> <div class="actionscontainer"> <fancy-dropdown inputid="actions" class="table-input"> <option value="none">Actions</option> <option value="Edit selected hosts">Edit selected images</option> <option value="Delete select hosts">Delete selected images</option> <option value="Delete select hosts">Deploy selected images</option> </fancy-dropdown> </div> <div class="os-container"> <div class="table"> <div class="tr"> <div class="th"><input type="checkbox"></div> <div class="th">Name</div> <div class="th">ISO</div> </div> <div class="tr" each="{img in this.opts.store.getState().iso_map}"> <div class="th" style="background-color: white"><input type="checkbox"></div> <div class="td">{img.os}</div> <div class="td">{img.file}</div> </div> </div> </div> </div> <add-button onclick="{addServerimage}">Add new Image</add-button>', 'serverimages-overview .os-container,[data-is="serverimages-overview"] .os-container{ background-color: white; padding: 34px 20px; } serverimages-overview .osgroup,[data-is="serverimages-overview"] .osgroup{ margin-bottom: 25px; margin-right: 25px; float: left; width: 100%; } serverimages-overview .osselect,[data-is="serverimages-overview"] .osselect{ background-color: #363c51; font-size: 0.95em; color: white; border: 1px solid #ecedf1; padding: 10px; width: 580px; } serverimages-overview .centeralign,[data-is="serverimages-overview"] .centeralign{ text-align: center; } serverimages-overview .tablewidth,[data-is="serverimages-overview"] .tablewidth{ width: 600px; }', '', function(opts) {
        let store = this.opts.store
...
riot.tag2('new-setting', '<div class="settings-container"> <fancy-textarea tag="Public Key" inputid="settings-view-key"> </fancy-textarea> </div> <fancy-button onclick="{addPublicKey}">Create</fancy-button> <fancy-button color="gray" onclick="{closeModal}">Cancel</fancy-button>', 'new-setting .settings-container,[data-is="new-setting"] .settings-container{ text-align: left; align-items: left; }', '', function(opts) {
        this.closeModal = function() {
            document.getElementById('modal-shadow').style.display = 'None';
        }.bind(this)
...
riot.tag2('settings', '<div class="settings-group"> <fancy-input tag="KUBAM IP Address" inputid="settings-view-ip" placeholder="{this.opts.store.getState().kubam_ip}"> </fancy-input> <fancy-button onclick="{updateIP}">Update</fancy-button> <div class="settings-container-big"> <div class="table keywidthtablelimit"> <div class="tr"> <div class="th">Public Keys</div> </div> <div class="tr" each="{k in this.opts.store.getState().keys}"> <div class="td">{k}</div> </div> </div> </div> </div> <add-button onclick="{addSetting}"> Add Setting </add-button>', 'settings .settings-group,[data-is="settings"] .settings-group{ padding: 15px; margin-bottom: 10px; background-color: white; } settings fancy-input,[data-is="settings"] fancy-input{ position: relative; float: left; top: -10px; } settings fancy-button,[data-is="settings"] fancy-button{ position: relative; top: 10px; } settings .keywidthtablelimit,[data-is="settings"] .keywidthtablelimit{ width: 100%; table-layout: fixed; word-wrap: break-word; }', '', function(opts) {
        let store = this.opts.store
