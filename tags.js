...
riot.tag2('settings', '<div class="settings-group"> <fancy-input tag="KUBAM IP Address" inputid="settings-view-ip" placeholder="{this.opts.store.getState().kubam_ip}"> </fancy-input> <fancy-button>Update</fancy-button> <div class="settings-container-big"> <div class="table"> <div class="tr"> <div class="th">Public Keys</div> </div> <div class="tr" each="{k in this.opts.store.getState().keys}"> <div class="td">{k.key}</div> </div> </div> </div> </div> <add-button onclick="{addSetting}"> Add Setting </add-button>', 'settings .settings-group,[data-is="settings"] .settings-group{ padding: 15px; margin-bottom: 10px; background-color: white; } settings fancy-input,[data-is="settings"] fancy-input{ position: relative; float: left; top: -10px; } settings fancy-button,[data-is="settings"] fancy-button{ position: relative; top: 10px; }', '', function(opts) {
        let store = this.opts.store
