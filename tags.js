...
riot.tag2('hosts', '<div class="container table-responsive"> <div class="top-actions"> <div class="dropdown"> <button class="btn btn-secondary dropdown-toggle" type="button" id="hostActions" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> Actions </button> <div class="dropdown-menu" aria-labelledby="hostActions"> <a class="dropdown-item dropdown-disabled" data-action="buildBootImages" onclick="{actionSelect}">Build Boot Images</a> <a class="dropdown-item dropdown-disabled" data-action="deleteHosts" onclick="{actionSelect}">Delete</a> <a class="dropdown-item dropdown-disabled" onclick="{actionSelect}">Deploy Service Profile</a> <a class="dropdown-item dropdown-disabled" onclick="{actionSelect}">Deploy VMedia Policy</a> </div> </div> </div> <table class="table table-bordered table-striped small"> <thead class="thead-dark"> <tr> <th scope="col"><input type="checkbox" id="select_all" onclick="{changeHostsSelection}"></th> <th scope="col">Hostname</th> <th scope="col">IP Address</th> <th scope="col">Operating System</th> <th scope="col">Role</th> <th scope="col">Network</th> <th scope="col">Server Group</th> <th scope="col">Server</th> <th scope="col">Service Profile Template</th> <th scope="col">Actions</th> </tr> </thead> <tbody> <tr each="{host, iindex in this.opts.store.getState().hosts}"> <td> <input type="checkbox" class="hostCheckBoxes form-control" data-hostname="{host.name}" onclick="{toggleCheck}" checked="{host.selected}"> </td> <td> <input type="text" class="form-control" riot-value="{host.name}" data-op="host" data-old="{host.name}" data-index="{iindex}" onblur="{changeHost}"> </td> <td> <input type="text" riot-value="{host.ip}" data-op="ip" data-old="{host.ip}" class="form-control" data-index="{iindex}" onblur="{changeHost}"> </td> <td> <select class="custom-select" data-attrib="os" onchange="{selectChange}"> <option each="{key, value in passStore.getState().catalog}" selected="{value === host.os}" data-old="{host.os}" riot-value="{value}" data-new="{value}" data-index="{iindex}" data-op="os">{translateOS(value)} </option> </select> </td> <td> <select class="custom-select" data-attrib="role" onchange="{selectChange}"> <option each="{value in passStore.getState().catalog != null ? passStore.getState().catalog[host.os] : []}" selected="{value=== host.role}" data-old="{host.role}" data-new="{value}" data-index="{iindex}" data-op="role"> {translateRole(value)} </option> </select> </td> <td> <select class="custom-select" data-attrib="network_group" onchange="{selectChange}"> <option each="{nw in passStore.getState().networks}" selected="{nw.name === host.network_group}" data-old="{host.network_group}" data-new="{nw.name}" data-index="{iindex}" data-op="network_group"> {nw.name} </option> </select> </td> <td> <select class="custom-select" data-attrib="server_group" onchange="{selectChange}"> <option each="{server in passStore.getState().servers}" selected="{server.name === host.server_group}" data-old="{host.server_group}" data-new="{server.name}" data-index="{iindex}" data-op="server_group"> {server.name} </option> </select> </td> <td> <select class="custom-select" data-attrib="server" onchange="{selectChange}"> <option each="{s in (passStore.getState().compute ? passStore.getState().compute[host.server_group] : [])}" selected="{s.server === host.server}" data-old="{host.server}" data-index="{iindex}" data-op="server"> {s.type === \'blade\' ? ⁗blade ⁗+s.chassis_id + ⁗/⁗+s.slot : ⁗Rack ⁗+s.rack_id} </option> </select> </td> <td> <select class="custom-select" data-attrib="sp_template" onchange="{selectChange}"> <option each="{s in (passStore.getState().templates ? passStore.getState().templates[host.server_group] : [{ ⁗name⁗ : host.service_profile_template}])}" selected="{host.service_profile_template === s.name}" data-old="{host.service_profile_template}" data-index="{iindex}" data-op="template"> {s.name} </option> </select> </td> <td> <img src="./icons/delete.svg" data-hostname="{host.name}" onclick="{deleteHost}" class="table-icon"> </td> </tr> </tbody> </table> </div> <add-button onclick="{addHost}">Add Host</add-button>', '', '', function(opts) {
        let currentValue
...
riot.tag2('settings', '<div class="container table-responsive"> <div class="input-group mb-3"> <div class="input-group-prepend"> <span class="input-group-text">KUBAM IP</span> </div> <input type="text" class="form-control" id="settings-view-ip" riot-value="{this.opts.store.getState().kubam_ip}" tip="This is the IP from which the servers will load the Vmedia. Usually it is the same IP as the web interface"> <button class="btn btn-outline-secondary" onclick="{updateIP}" type="button">Update</button> </div> <table class="table table-bordered table-striped keywidthtablelimit small"> <thead class="thead-dark"> <tr> <th scope="col" width="90%">Public Keys</th> <th scope="col">Actions</th> </tr> </thead> <tbody> <tr each="{k in this.opts.store.getState().keys}"> <td><span class="small">{k}</span></td> <td> <img src="./icons/edit.svg" class="table-icon" data-id="{k}" onclick="{editSetting}"> <img src="./icons/delete.svg" data-id="{k}" onclick="{deleteKey}" class="table-icon"> </td> </tr> </tbody> </table> </div> <add-button onclick="{addSetting}"> Add Keys </add-button>', 'settings .keywidthtablelimit,[data-is="settings"] .keywidthtablelimit{ width: 100%; word-wrap: break-word; table-layout: fixed; }', '', function(opts) {
        let store = this.opts.store
...
riot.tag2('top-bar', '<nav class="navbar navbar-dark bg-dark flex-md-nowrap p-0"> <a class="navbar-brand col-sm-3 col-md-2 mr-0" href="#hosts"> <img src="icons/cisco.png"> &nbsp; &nbsp; &nbsp; KUBAM </a> <span class="navbar-text mr-auto" id="currentpage"> &nbsp; &nbsp; <router> <route path="network..">Network Groups</route> <route path="images..">Images</route> <route path="infrastructure">Infrastructure</route> <route path="infrastructure/*"><inner-tag></inner-tag></route> <route path="hosts..">Hosts</route> <route path="settings..">Settings</route> <route path="tutorial..">Tutorial</route> <route path="feedback..">Feedback</route> </router> </span> <ul class="navbar-nav ml-auto"> <li class="nav-item"> <a href="https://ciscoucs.github.io/kubam"> <img src="icons/tutorial.png"> </a> </li> <li class="nav-item"> <a href="https://github.com/CiscoUcs/KUBaM"> <img src="icons/github.png"> </a> </li> </ul> </nav>', '', '', function(opts) {
});

riot.tag2('inner-tag', '<span class="topb"><a href="#infrastructure" onclick="route(\'infrastructure\')">Infrastructure</a> | {name}</span>', '', '', function(opts) {
